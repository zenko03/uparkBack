...
public class ReservationService {
    ...
    private ReservationStatus getStatusByValue(int value) {
        ...
        List<ReservationStatus> allStatuses = reservationStatusRepository.findAll();
    ...
    private ReservationStatus getDefaultReservationStatus() {
        // Récupérer le statut "En attente" (value_ = 10) par défaut
        List<ReservationStatus> allStatuses = reservationStatusRepository.findAll();
        return allStatuses.stream()
                .filter(status -> status.getValue() == 10) // "En attente"
                .findFirst()
