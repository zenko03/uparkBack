...
public class ReservationService {
    ...
    public Reservation createReservation(ReservationRequest request) {
        ...
        if (request.getVehicles() != null) {
            for (VehicleQuantity vehicleQuantity : request.getVehicles()) {
                createReservationVehicles(savedReservation, vehicleQuantity, request.getParkingId());
            }
        }
    ...
    public BigDecimal calculateTotalPrice(PriceCalculationRequest request) {
        ...
        int totalVehicles = request.getVehicles() != null ? 
                request.getVehicles().stream().mapToInt(VehicleQuantity::getQuantity).sum() : 1;
    ...
    private void createReservationVehicles(Reservation reservation, VehicleQuantity vehicleQuantity, int parkingId) {
        // Trouver l'annonce véhicule correspondante (logique simplifiée)
        // Dans un vrai cas, il faudrait une logique plus complexe pour matcher avec Announcements_vehicles
        ReservationVehicles reservationVehicles = ReservationVehicles.builder()
                .reservation(reservation)
                .numbers(vehicleQuantity.getQuantity())
                .build();
